...
> **開発中 (M1)** — API は動作しますが、画面はまだありません。
> 現時点で利用できるのは REST API のみです。
...
| マイルストーン | 内容                                       | 状態                 |
| -------------- | ------------------------------------------ | -------------------- |
| M0             | 開発基盤、CI、マイグレーション、E2E 骨組み | 完了                 |
| **M1**         | アカウント・組織・権限                     | API 完了 / UI 未着手 |
...
M1 の受け入れシナリオ（空のデータベースから、オーナー作成 → 第一営業所作成 →
管理者作成 → 管理者が管理者を追加 → メンバー作成 → 部署への配属 → 複数端末ログイン
→ 停止と復元）は [e2e/tests/m1-acceptance.spec.ts](e2e/tests/m1-acceptance.spec.ts)
で自動テスト化されています。
